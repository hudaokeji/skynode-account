...
function mountRouter(expressApp) {
  ...
  const features = expressApp.features = require('./features');
...
//      options     = require('./config'),
      ...
      modelsStore      = require('./models').createModels(store),
...
if (app.mailer) {
  require("./email/quick-send").initMailer(app.mailer);
}

// Define some global template variables.
var helpers2 = helpers.createHelpers(app);
app.locals.version = app.get('version');
app.locals.client = app.get('client');
Object.keys(helpers2).forEach(function (key) {
  app.locals[key] = helpers2[key];
});
