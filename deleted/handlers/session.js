...
*/
var utils = require("skynode-server").utils;
var errors = require("skynode-server").errors;
...
module.exports = Observable.extend({
  ...
  deleteAccount: function (req, res, next) {
    // first validate their username matches the name given
    if (req.body.confirm.trim().toLowerCase() !== req.session.user.name.trim().toLowerCase()) {
      res.flash(req.flash.ERROR, 'The confirmation username did not match your username, no dice I\'m afraid.');
      return res.redirect('back');
    }

    if (req.session.user.pro) {
      // cancel their pro subscription also
      require('./upgrade').cancel(req, res, function () {
        // do nothing
      });
    }

    this.models.user.delete(req.session.user.name, function (error) {
      if (error) {
        console.log(error.stack);
        res.flash(req.flash.ERROR, 'Something went wrong I\'m afraid');
        return res.redirect('back');
      }

      res.flash(req.flash.ERROR, 'Your account is now fully deleted.');
  ...
  updateUser: function (req, res, next) {
    ...
    var updateOwnershipData = denodeify(session.models.user.updateOwnershipData).bind(session.models.user);
    var updateKey = denodeify(session.models.user.updateKey).bind(session.models.user);
    ...
    var createToken = function createToken(user) {
      this.models.forgotToken.createToken(user.name, function (err, token) {
        ...
        var ctx = {
          ...
          link: helpers.url('reset?token=' + token, true, true),
          user: user,
        };
  ...
  resetPassword: function (req, res) {
    ...
    this.models.forgotToken.loadUser(token, function (err, user) {
      if (err) {
        res.flash(req.flash.ERROR, 'This token has failed, please use the reset token within 24 hours of creation');
        return res.redirect(303, _this.helpers.url());
      }

      if (!user) {
        res.flash(req.flash.ERROR, 'This token has expired, please use the reset token within 24 hours of creation');
        return res.redirect(303, _this.helpers.url());
      }
